...
class GraphQLStorage implements IStorage {
  ...
  // Authors management - from GraphQL
  ...
  async getAuthor(id: string): Promise<Author | undefined> {
    ...
    try {
      const data = await graphqlRequest(query, { id });
      return data.libaray_Autor_by_pk;
    } catch (error) {
  ...
  async createAuthor(author: InsertAuthor): Promise<Author> {
    const mutation = `
      mutation CreateAuthor($author: libaray_Autor_insert_input!) {
        insert_libaray_Autor_one(object: $author) {
          id
          name
          bio
          book_num
  ...
  async updateAuthor(id: string, author: UpdateAuthor): Promise<Author | undefined> {
    const mutation = `
      mutation UpdateAuthor($id: uuid!, $author: libaray_Autor_set_input!) {
        update_libaray_Autor_by_pk(pk_columns: {id: $id}, _set: $author) {
          id
          name
          bio
          birth_date
    ...
    try {
      const data = await graphqlRequest(mutation, { id, author });
  ...
  async getBook(id: string): Promise<Book | undefined> {
    ...
    try {
      const data = await graphqlRequest(query, { id });
      return data.libaray_Book_by_pk;
    } catch (error) {
  ...
  async createBook(book: InsertBook): Promise<Book> {
    ...
    const variables = {
      ...
      chapter_num: book.parts_num,
      ISBN: parseInt(book.ISBN) || 0,
